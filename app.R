...
server <- function(input, output, session) {
    ...
    rNodeData <- reactiveFileReader(1000,NULL,"data.xlsx", read_excel,sheet=1)
    rEdgeData <- reactiveFileReader(1000,NULL,"data.xlsx", read_excel,sheet=2)
    ...
    graphData$nodeLegend <- reactive({
      data.frame(color = iwanthue(max(graphData$groups),80,309,27,100,5,55)) %>%
